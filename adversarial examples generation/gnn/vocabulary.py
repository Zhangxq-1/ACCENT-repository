...
def train_vocab(size):
    corpus=[]
    f_code=open('/dataset/java/train/code.original','r')
    ...
    if not os.path.exists('./embedding/java/train'):
        os.mkdir('./embedding/train')

    w2v = Word2Vec(corpus, size=size, workers=16, sg=1, min_count=1)
    w2v.save('./embedding/java/train/node_w2v_64')
