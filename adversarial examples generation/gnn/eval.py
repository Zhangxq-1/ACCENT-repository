...
class Test(object):
    ...
    def test_for_adv(self,edge_batch):
        for index_batch, batch in enumerate(self.dataloader):
            ...
            '''
            s_blue_score,encoder=self.test_one_batch(batch, edge_batch, batch_size)
            return s_blue_score,encoder
            '''
    #----------------------------- not used -----------------------------------------------------
    ...
    def run_test(self) -> dict:
        ...
        """
        avg_s_bleu, avg_meteor = self.test_iter()
        scores_dict = {
            's_bleu': avg_s_bleu,
            'meteor': avg_meteor
        }
    ...
    #-----------------not used in adversarial attack ---------------------------
    def test_iter(self):
        """
        evaluate model on self.dataset
        :return: scores
