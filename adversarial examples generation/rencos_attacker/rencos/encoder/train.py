...
def get_batch(dataset,idx,bs):
    ...
    src = torch.LongTensor(src).view(configuation.max_length,32)

    trg=torch.LongTensor(trg).view(30,32)
    return src,trg
...
def word2index(word,mode):
    ...
    if mode == 'src':
        ...
        if len(index_all_var)>=configuation.max_length:
            index_all_var=index_all_var[0:configuation.max_length]
        else:
            for i in range(configuation.max_length-len(index_all_var)):
                index_all_var.append(max_token_src)
    else:
        for item in word:
        ...
        if len(index_all_var)>=30:
            index_all_var=index_all_var[0:30]
        else:
            for i in range(30-len(index_all_var)):
                index_all_var.append(max_token_trg)
...
def train(model, train_data, optimizer, criterion):
    ...
    for epoch in range(EPOCHES):
        ...
        if avg_loss < old_loss:
            torch.save(model.state_dict(),'./python_model.pkl')
            old_loss=avg_loss
...
if __name__ == '__main__':
    ...
    data=pd.read_pickle('.python_train_data.pkl')
    print('read embedding....')
    root = './'
    word2vec_src = Word2Vec.load(root + 'python_vocab/python_node_w2v_code_64').wv
    ...
    word2vec_trg = Word2Vec.load(root + 'python_vocab/python_node_w2v_summ_64').wv
