...
class Translator(object):
    ...
    def __init__(self,
        ...
        self.prob_files = ["/media/zxq/5d22d1db-ba73-4f1c-8284-d9bde84bb875/zxq/rencos_attacker/rencos/samples/%s/test/prs.%d"%(opt.lang, r) for r in range(self.refer)] if self.refer else []
    ...
    def translate(self,
        ...
        #这里的data 还是对 src ref tgt三个的封装，每一个的本质如下
         #(src_examples_iter, num_src_feats)  = (examples_iter, num_feats) 即实际内容为( ({}, {}, {}, {},{}), num_feats)
     #每一个字典对：
     #{'src': ('protected', 'boolean', '[', ']', 'dataset', 'Integrity', '(', 'boolean', 'nominal', 'Predictor', ',', 'boolean', 'numeric', 'Predictor', ',', 'boolean', 'string', 'Predictor', ',', 'boolean', 'date', 'Predictor', ',', 'boolean', 'relational', 'Predictor', ',', 'boolean', 'multi', 'Instance', ',', 'int', 'class', 'Type', ',', 'boolean', 'predictor', 'Missing', ',', 'boolean', 'class', 'Missing', ')', '{', 'print', '(', '_', 'STR', ')', ';', 'print', 'Attribute', 'Summary', '(', 'nominal', 'Predictor', ',', 'numeric', 'Predictor', ',', 'string', 'Predictor', ',', 'date', 'Predictor', ',', 'relational', 'Predictor', ',', 'multi', 'Instance', ',', 'class', 'Type', ')', ';', 'print', '(', '_', 'STR', ')', ';', 'int', 'num', 'Train', '=', 'get', 'Num', 'Instances', '(', ')', ',', 'num', 'Classes', '=', '_', 'NUM', ',', 'missing', 'Level', '=', '_', 'NUM', ';', 'boolean', '[', ']', 'result', '=', 'new', 'boolean', '[', '_', 'NUM', ']', ';', 'Instances', 'train', '=', 'null', ';', 'Kernel', 'kernel', '=', 'null', ';', 'try', '{', 'train', '=', 'make', 'Test', 'Dataset', '(', '_', 'NUM', ',', 'num', 'Train', ',', 'nominal', 'Predictor', '?', 'get', 'Num', 'Nominal', '(', ')', ':', '_', 'NUM', ',', 'numeric', 'Predictor', '?', 'get', 'Num', 'Numeric', '(', ')', ':', '_', 'NUM', ',', 'string', 'Predictor', '?', 'get', 'Num', 'String', '(', ')', ':', '_', 'NUM', ',', 'date', 'Predictor', '?', 'get', 'Num', 'Date', '(', ')', ':', '_', 'NUM', ',', 'relational', 'Predictor', '?', 'get', 'Num', 'Relational', '(', ')', ':', '_', 'NUM', ',', 'num', 'Classes', ',', 'class', 'Type', ',', 'multi', 'Instance', ')', ';', 'if', '(', 'missing', 'Level', '>', '_', 'NUM', ')', '{', 'add', 'Missing', '(', 'train', ',', 'missing', 'Level', ',', 'predictor', 'Missing', ',', 'class', 'Missing', ')', ';', '}', 'kernel', '=', 'Kernel', '.', 'make', 'Copies', '(', 'get', 'Kernel', '(', ')', ',', '_', 'NUM', ')', '[', '_', 'NUM', ']', ';', '}', 'catch', '(', 'Exception', 'ex', ')', '{', 'throw', 'new', 'Error', '(', '_', 'STR', '+', 'ex', '.', 'get', 'Message', '(', ')', ')', ';', '}', 'try', '{', 'Instances', 'train', 'Copy', '=', 'new', 'Instances',
      #   '(', 'train', ')', ';', 'kernel', '.', 'build', 'Kernel', '(', 'train', 'Copy', ')', ';', 'compare'), 'indices': 0}
        ...
        data_iter = inputters.OrderedIterator(     
            #这里返回的就是tensor了 是通过for b in torchtext.data.batch(self.data(), self.batch_size,
                                                            #self.batch_size_fn): 这个来实现变化的
        ...
        for batch in data_iter:
            ...
            batch_data = self.translate_batch(batch, data, attn_debug,
                                              fast=self.fast)  #解码结果
            translations = builder.from_batch(batch_data)   #翻译结果
    ...
    def _run_encoder(self, batch, data_type):
        ...
        #print('src')
        #print(src.shape)   torch.Size([300, 32, 1])
        #print('src_length')
        #print(src_lengths.shape)   torch.Size([32])
        #exit()
        enc_states, memory_bank, src_lengths = self.model.encoder(
            src, src_lengths)
        if src_lengths is None:
            #print('src_lengths is None')
    ...
    def _fast_translate_batch(self,
        ...
        [.src]:('[torch.cuda.LongTensor of size 300x32 (GPU 0)]', '[torch.cuda.LongTensor of size 32 (GPU 0)]')
        [.indices]:[torch.cuda.LongTensor of size 32 (GPU 0)]
        [.ref0]:('[torch.cuda.LongTensor of size 300x32 (GPU 0)]', '[torch.cuda.LongTensor of size 32 (GPU 0)]')
        [.ref1]:('[torch.cuda.LongTensor of size 300x32 (GPU 0)]', '[torch.cuda.LongTensor of size 32 (GPU 0)]')
