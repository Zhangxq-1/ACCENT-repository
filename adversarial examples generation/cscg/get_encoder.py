...
def pooling(x):
    ...
    out=nn.MaxPool1d(8,stride=8)   ##CHANGE DIM
    x=out(x)
    x=x.squeeze()
    return x
...
def return_encoder(src,trg,model,vocab_src,vocab_trg,max_token_src,max_token_trg):
    ...
    src_index = torch.LongTensor(src_index).view(150, 1)

    #trg_index = [word2index(trg, 'trg',vocab_src,vocab_trg,max_token_src,max_token_trg)]
    #trg_index = torch.LongTensor(trg_index).view(150, 1)
