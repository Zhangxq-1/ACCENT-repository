...
def load_data(args, filenames, max_examples=-1, dataset_name='deepcom',
    ...
    ############
    with open(filenames['original']) as f:
        originals=[line.strip() for line in 
                   tqdm(f, total=count_file_lines(filenames['original']))]
    ...
    assert len(sources) == len(source_tags) == len(targets)==len(originals)

    examples = []
    #####################
    count_id=0
    for src, src_tag, tgt,src_ori in tqdm(zip(sources, source_tags, targets,originals),
                                  total=len(sources)):
