...
#####################################################
    def update(self, ex):
        ...
        loss_ori = net_loss['ml_loss'].mean() if self.parallel \
            else net_loss['ml_loss']
        loss_per_token_ori = net_loss['loss_per_token'].mean() if self.parallel \
            else net_loss['loss_per_token']
        ml_loss_ori = loss_ori.item()
        loss_per_token_ori = loss_per_token_ori.item()
        loss_per_token_ori = 10 if loss_per_token_ori > 10 else loss_per_token_ori
        perplexity_ori = math.exp(loss_per_token_ori)
            #####################################3
            ##############################
            #
            ##
            #
        adv_ex=self.adversary.generate(ex,self.src_dict,self.tgt_dict)
        
        #-------------------------------------------------------------------------------------
        adv_source_map, adv_alignment = None, None
        adv_blank, adv_fill = None, None
        # To enable copy attn, collect source map and alignment info
        if self.args.copy_attn:
            assert 'src_map' in adv_ex and 'alignment' in adv_ex

            adv_source_map = make_src_map(adv_ex['src_map'])
            adv_source_map = adv_source_map.cuda(non_blocking=True) if self.use_cuda \
                else adv_source_map

            adv_alignment = align(adv_ex['alignment'])
            adv_alignment = adv_alignment.cuda(non_blocking=True) if self.use_cuda \
                else adv_alignment

            adv_blank, adv_fill = collapse_copy_scores(self.tgt_dict, ex['src_vocab'])

        adv_code_word_rep = adv_ex['code_word_rep']
        adv_code_char_rep = adv_ex['code_char_rep']
        adv_code_type_rep = adv_ex['code_type_rep']
        adv_code_mask_rep = adv_ex['code_mask_rep']
        adv_code_len = adv_ex['code_len']
        adv_summ_word_rep = adv_ex['summ_word_rep']
        adv_summ_char_rep = adv_ex['summ_char_rep']
        adv_summ_len = adv_ex['summ_len']
        adv_tgt_seq = adv_ex['tgt_seq']

        if self.use_cuda:
            adv_code_len = adv_code_len.cuda(non_blocking=True)
            adv_summ_len = adv_summ_len.cuda(non_blocking=True)
            adv_tgt_seq = adv_tgt_seq.cuda(non_blocking=True)
            if adv_code_word_rep is not None:
                adv_code_word_rep = adv_code_word_rep.cuda(non_blocking=True)
            if adv_code_char_rep is not None:
                adv_code_char_rep = adv_code_char_rep.cuda(non_blocking=True)
            if adv_code_type_rep is not None:
                adv_code_type_rep = adv_code_type_rep.cuda(non_blocking=True)
            if adv_code_mask_rep is not None:
                adv_code_mask_rep = adv_code_mask_rep.cuda(non_blocking=True)
            if adv_summ_word_rep is not None:
                adv_summ_word_rep = adv_summ_word_rep.cuda(non_blocking=True)
            if adv_summ_char_rep is not None:
                adv_summ_char_rep = adv_summ_char_rep.cuda(non_blocking=True)
            if ex_weights is not None:
                ex_weights = ex_weights.cuda(non_blocking=True)
        
        adv_net_loss = self.network(code_word_rep=adv_code_word_rep,
                                    code_char_rep=adv_code_char_rep,
                                    code_type_rep=adv_code_type_rep,
                                code_len=adv_code_len,
                                summ_word_rep=adv_summ_word_rep,
                                summ_char_rep=adv_summ_char_rep,
                                summ_len=adv_summ_len,
                                tgt_seq=adv_tgt_seq,
                                src_map=adv_source_map,
                                alignment=adv_alignment,
                                src_dict=self.src_dict,
                                tgt_dict=self.tgt_dict,
                                max_len=self.args.max_tgt_len,
                                blank=adv_blank,
                                fill=adv_fill,
                                source_vocab=adv_ex['src_vocab'],
                                code_mask_rep=adv_code_mask_rep,
                                example_weights=ex_weights)

        loss_adv = adv_net_loss['ml_loss'].mean() if self.parallel \
            else adv_net_loss['ml_loss']
        loss_per_token_adv = adv_net_loss['loss_per_token'].mean() if self.parallel \
            else adv_net_loss['loss_per_token']
        ml_loss_adv = loss_adv.item()
        loss_per_token_adv = loss_per_token_adv.item()
        loss_per_token_adv = 10 if loss_per_token_adv > 10 else loss_per_token_adv
        perplexity_adv = math.exp(loss_per_token_adv)
        
        loss=0.7*loss_ori+0.3*loss_adv
        #------------------------
        ...
        return {
            'ml_loss': 0.7*ml_loss_ori+0.3*ml_loss_adv,
            'perplexity': 0.7*perplexity_ori+0.3*perplexity_adv
        }
