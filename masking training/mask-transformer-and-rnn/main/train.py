...
def add_train_args(parser):
    ...
    files.add_argument('--train_tgt', nargs='+', type=str,
       ...
       #################################3333                
    files.add_argument('--train_ori',nargs='+',type=str,
            default='/home/zxq/code/adversarial_NCS/data/java/train/code.original')
    ...
    files.add_argument('--dev_ori', nargs='+', type=str, required=True,
                       default='/home/zxq/code/adversarial_NCS/data/java/dev/code.original')
...
def set_defaults(args):
    ...
    if not args.only_test:
        ...
        for i in range(num_dataset):
            ...
            train_tgt = os.path.join(data_dir, args.train_tgt[i])
                ####################
            train_original=os.path.join(data_dir,args.train_ori[i])
            ...
            if not os.path.isfile(train_original):
                raise IOError('No such file: %s' % train_original)
            ...
            ##############################
            args.train_original_files.append(train_original)

    args.dev_src_files = []
    args.dev_tgt_files = []
    args.dev_src_tag_files = []
    args.dev_original_files=[]
    ...
    for i in range(num_dataset):
        ...
        dev_original=os.path.join(data_dir,args.dev_ori[i])
        ...
        args.dev_original_files.append(dev_original)
...
def main(args):
    ...
    if not args.only_test:
        ...
        ##############################################################33
        for train_src, train_src_tag, train_tgt,train_original, dataset_name in \
                zip(args.train_src_files, args.train_src_tag_files,
                    args.train_tgt_files,args.train_original_files, args.dataset_name):
            train_files = dict()
            train_files['src'] = train_src
            train_files['src_tag'] = train_src_tag
            train_files['tgt'] = train_tgt
            ##############
            train_files['original']=train_original
    ...
    dev_exs = []
    for dev_src, dev_src_tag, dev_tgt,dev_original, dataset_name in \
            zip(args.dev_src_files, args.dev_src_tag_files,
                args.dev_tgt_files,args.dev_original_files, args.dataset_name):
        dev_files = dict()
        dev_files['src'] = dev_src
        dev_files['src_tag'] = dev_src_tag
        dev_files['tgt'] = dev_tgt

        dev_files['original']=dev_original
    ...
    # TRAIN/VALID LOOP
        ...
        for epoch in range(start_epoch, args.num_epochs + 1):
            ...
            #------------------------adv vode--------------------------------
            #validate_official(args,dev_loader_adv,model,stats)
            # Save best valid
