...
import pandas as pd
import numpy as np
from adversary2 import Adversary
...
class Train(object):
    ...
    def __init__(self, vocab_file_path=None, model_file_path=None):
        ...
        self.train_dataloader = DataLoader(dataset=self.train_dataset,
                                           ...
                                           shuffle=False,
                                           collate_fn=lambda *args: utils.collate_fn_adv(args,
        ...
        self.adversary=Adversary(config.var_everyCode_path,config.original_code_path)
        ...
        if model_file_path:
            ...
            self.lr_scheduler=lr_scheduler.StepLR(self.optimizer,step_size=config.lr_decay_every,gamma=config.lr_decay_rate,
                last_epoch=123
            )
    ...
    def train_one_batch(self, batch: utils.Batch,adv_batch: utils.Batch, edge_batch, batch_size, criterion):
        """
        train one batch
        ...
        loss_origin = criterion(decoder_outputs, nl_batch)
        #------------------adv---------------------------------------------

        adv_decoder_outputs = self.model(adv_batch, batch_size, self.nl_vocab, edge_batch)     # [T, B, nl_vocab_size]

        batch_nl_vocab_size = adv_decoder_outputs.size()[2]     # config.nl_vocab_size (+ max_oov_num)
        adv_decoder_outputs = adv_decoder_outputs.view(-1, batch_nl_vocab_size)
        nl_batch = nl_batch.view(-1)

        loss_adv = criterion(adv_decoder_outputs, nl_batch)
        loss=(1-config.lamta*config.lamta_decay)*loss_origin+config.lamta*config.lamta_decay*loss_adv
    ...
    def get_batch(self, edages_data, idx, bs):
        #print(len(edages_data))
        #print(idx)
        #print(bs)
        tmp = edages_data.iloc[(idx-1)*config.batch_size: idx*config.batch_size]
        x1 = []
    ...
    def train_iter(self):
        ...
        for epoch in range(0,config.n_epochs):
            ...
            last_print_index = 0
            last_plot_index = 0
            for index_batch, batch_data in enumerate(self.train_dataloader):

                batch_size = config.batch_size
                if index_batch == 0 :#or index_batch == 2178:
                ...
                elif index_batch == 720:#180--bs=32 :# or index_batch == 2358:
                    path_edge = '/dataset/java/train/adj1.pkl'
                    f_edge = open(path_edge,'rb')
                    edages_data = pickle.load(f_edge)
                    f_edge.close()
                    index = 0
                elif index_batch == 1440:#360:# or index_batch == 2538:
                    path_edge = '/dataset/java/train/adj2.pkl'
                    f_edge = open(path_edge,'rb')
                    edages_data = pickle.load(f_edge)
                    f_edge.close()
                    index = 0
                elif index_batch == 2160:#540 :#or index_batch == 2718:
                    path_edge = '/dataset/java/train/adj3.pkl'
                    f_edge = open(path_edge,'rb')
                    edages_data = pickle.load(f_edge)
                    f_edge.close()
                    index = 0
                elif index_batch == 2880:#720 :#or index_batch == 2898:
                    path_edge = '/dataset/java/train/adj4.pkl'
                    f_edge = open(path_edge,'rb')
                    edages_data = pickle.load(f_edge)
                    f_edge.close()
                    index = 0
                elif index_batch == 3600:#900 :#or index_batch == 3078:
                    path_edge = '/dataset/java/train/adj5.pkl'
                    f_edge = open(path_edge,'rb')
                    edages_data = pickle.load(f_edge)
                    f_edge.close()
                    index = 0
                elif index_batch == 4320:#1080 :#or index_batch == 3258:
                    path_edge = '/dataset/java/train/adj6.pkl'
                    f_edge = open(path_edge,'rb')
                    edages_data = pickle.load(f_edge)
                    f_edge.close()
                    index = 0
                elif index_batch == 5040:#1260 :#or index_batch == 3438:
                    path_edge = '/dataset/java/train/adj7.pkl'
                    f_edge = open(path_edge,'rb')
                    edages_data = pickle.load(f_edge)
                    f_edge.close()
                    index = 0
                elif index_batch == 5760:#1440 :#or index_batch == 3618:
                    path_edge = '/dataset/java/train/adj8.pkl'
                    f_edge = open(path_edge,'rb')
                    edages_data = pickle.load(f_edge)
                    f_edge.close()
                    index = 0
                elif index_batch == 6480:#1620 :#or index_batch == 3798:
                    path_edge = '/dataset/java/train/adj9.pkl'
                    f_edge = open(path_edge,'rb')
                    edages_data = pickle.load(f_edge)
                    f_edge.close()
                    index = 0
                elif index_batch == 7200:#1800 :#or index_batch == 3978:
                    path_edge = '/dataset/java/train/adj10.pkl'
                    f_edge = open(path_edge,'rb')
                    edages_data = pickle.load(f_edge)
                    f_edge.close()
                    index = 0
                elif index_batch == 7920:#1980 :#or index_batch == 4158:
                ...
                ###
                #####
                #batch_data is [source_batch,code_batch,nl_batch] 
                # each of item is a list eg:source_batch is a batch of source

              
                batch=utils.word2index_adv(batch_data,self.source_vocab,self.code_vocab,self.nl_vocab)
                ...
                adv_batch_data=self.adversary.generate_example(batch_data,edge_batch,index_batch,batch_size)
                adv_batch=utils.word2index_adv(adv_batch_data,self.source_vocab,self.code_vocab,self.nl_vocab)

             
                loss = self.train_one_batch(batch,adv_batch, edge_batch, batch_size, criterion)
