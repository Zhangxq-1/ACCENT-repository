...
def read_corpus(file_path, source):
    ...
    for line in open(file_path):
        ...
        sent = [w.encode('utf-8').decode("utf-8") for w in sent]
...
def batch_slice_for_adv(data, batch_size, sort=True):
    batched_data = []
    batch_num = int(np.ceil(len(data) / float(batch_size)))
    for i in range(batch_num):
        cur_batch_size = batch_size if i < batch_num - 1 else len(data) - batch_size * i
        src_sents = [data[i * batch_size + b][0] for b in range(cur_batch_size)]
        tgt_sents = [data[i * batch_size + b][1] for b in range(cur_batch_size)]
        ori_sents=[data[i * batch_size + b][2] for b in range(cur_batch_size)]
        var_sents=[data[i * batch_size + b][3] for b in range(cur_batch_size)]

        if sort:
            src_ids = sorted(range(cur_batch_size), key=lambda src_id: len(src_sents[src_id]), reverse=True)
            src_sents = [src_sents[src_id] for src_id in src_ids]
            tgt_sents = [tgt_sents[src_id] for src_id in src_ids]
            ori_sents=[ori_sents[src_id] for src_id in src_ids]
            var_sents=[var_sents[src_id] for src_id in src_ids]

        batched_data.append((src_sents, tgt_sents,ori_sents,var_sents))

    return batched_data



def data_iter_for_adv(data, batch_size, shuffle=True):
    """
    randomly permute data, then sort by source length, and partition into batches
    ensure that the length of source sentences in each batch is decreasing
    """
    ...
    for src_len in buckets:
        tuples = buckets[src_len]
        if shuffle: np.random.shuffle(tuples)
        batched_data.extend(batch_slice_for_adv(tuples, batch_size))
