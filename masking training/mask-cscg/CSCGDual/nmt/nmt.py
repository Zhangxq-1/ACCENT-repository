...
from nmt.adversary import generate_for_c2nl,generate_for_nl2c
from nltk.translate.bleu_score import corpus_bleu, sentence_bleu, SmoothingFunction

from nmt.util import read_corpus, data_iter,data_iter_for_adv
...
def init_config():
    ...
    parser.add_argument('--train_ori', type=str, default=None,help='path to the training original file')
   # parser.add_argument('--dev_ori', type=str, help='path to the dev original file')
    parser.add_argument('--process', type=str,default=None, help='c2nl/nl2c/none //none for others')
    ...
    def translate(self, src_sents, beam_size=None, to_word=True):
        ...
        if to_word:
            for i, hyp in enumerate(completed_hypotheses):
                #print(hyp)
                hyp[1:]=[hyp[i].item() for i in range(1,len(hyp))]
                completed_hypotheses[i] = [self.vocab.tgt.id2word[w] for w in hyp]
                #print(completed_hypotheses[i])
...
import pandas as pd
def load_pkl(path='/data/java/var_for_everyCode.pkl'):
...
def train(args):
    ...
    #load pickle  -----
    var_everyCode_list=load_pkl()['variable'].tolist()
    train_data_ori=read_corpus(args.train_ori,source='original')
    train_data = list(zip(train_data_src, train_data_tgt,train_data_ori,var_everyCode_list))
    ...
    while True:
        ...
        for src_sents, tgt_sents,ori_sents,var_list in data_iter_for_adv(train_data, batch_size=args.batch_size):
            train_iter += 1
            ...
            if args.process=='c2nl':
                adv_src_sents=generate_for_c2nl(ori_sents,var_list)
                # call generate_adv_c2nl
            if args.process=='nl2c':
                adv_src_sents=generate_for_nl2c(src_sents)
                #call generate_adv_nl2c
            adv_src_sents_len = [len(s) for s in adv_src_sents]
            sorted_id=sorted(range(len(adv_src_sents_len)),key=lambda k: adv_src_sents_len[k],reverse=True)
            adv_src_sents_sort=[adv_src_sents[i] for i in sorted_id]
            tgt_sents_sort=[tgt_sents[i] for i in sorted_id]
            adv_src_sents_len_sort=[adv_src_sents_len[i] for i in sorted_id]
            adv_src_sents_len=adv_src_sents_len_sort


            adv_src_sents_var = to_input_variable(adv_src_sents_sort, vocab.src, cuda=args.cuda)
            adv_tgt_sents_var = to_input_variable(tgt_sents_sort, vocab.tgt, cuda=args.cuda)
            
            
            


            adv_scores, _ = model(adv_src_sents_var, adv_src_sents_len, adv_tgt_sents_var[:-1])
            # if args.cuda:
            #     tgt_sents_var = tgt_sents_var.cuda()
            adv_word_loss = cross_entropy_loss(adv_scores.view(-1, adv_scores.size(2)), adv_tgt_sents_var[1:].view(-1))
            adv_loss = adv_word_loss / batch_size
            adv_word_loss_val = adv_word_loss.item()
            adv_loss_val = adv_loss.item()
            if epoch <=10:
                loss_add=1*loss+0*adv_loss
                word_loss_val_add=1*word_loss_val+0*adv_word_loss_val

            elif epoch>10 and epoch<=20:
                loss_add=0.95*loss+0.05*adv_loss
                word_loss_val_add=0.95*word_loss_val+0.05*adv_word_loss_val
            elif epoch>20 and epoch<=30:
                loss_add=0.9*loss+0.1*adv_loss
                word_loss_val_add=0.9*word_loss_val+0.1*adv_word_loss_val
            else:
                loss_add=0.8*loss+0.2*adv_loss
                word_loss_val_add=0.8*word_loss_val+0.2*adv_word_loss_val
            ...
            report_loss += word_loss_val_add
            cum_loss += word_loss_val_add
...
def get_bleu(references, hypotheses, state):
    ...
    print(bleu_score/len(hypotheses))
...
def test(args):
    ...
    bleu_score = get_bleu([tgt for tgt in test_data_tgt], top_hypotheses, 'test')
    ...
    word_acc = get_acc([tgt for tgt in test_data_tgt], top_hypotheses, 'word_acc')
    sent_acc = get_acc([tgt for tgt in test_data_tgt], top_hypotheses, 'sent_acc')
